...
// .w   = random value (0-1)
//
Shader "Kvant/Wall/Kernel"
{
	Properties
    {
        _ParallaxMap("Height map", 2D) = "white" {}
        _Parallax("Height", Range(0.0, 40.0)) = 5
    }
    ...
    float4 frag_position(v2f_img i) : SV_Target
    {
        ...
        // Displacement
    #if POSITION_Z
        float h = tex2Dlod(_ParallaxMap, float4(i.uv, 0, 0)).a * _Parallax;
        float3 disp = float3(0, 0, h) + float3(0, 0, snoise(nc)) ;
